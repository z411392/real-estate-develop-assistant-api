...
class OwnershipRepository:
    ...
    async def get(self, ownershipId: str):
        documentSnapshots: DocumentSnapshot = await self._collection.document(ownershipId).get(transaction=self._transaction)
        return Ownership.fromDocumentSnapshot(documentSnapshots) if documentSnapshots.exists else None
