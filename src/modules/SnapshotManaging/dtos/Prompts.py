...
// 以「共有部分：民有段00304-000建號****429.04平方公尺」為例
type 共有部分 = {
    地段: string // 民有段
    小段?: string
    建號: string // 00304-000
...
```
並請注意：
1. 如果內容是面積，請換算為平方公尺並只保留數值部分，最後用字串儲存。
2. 如果有全形的數字、符號，請轉為半形。
...
並請注意：
1. 如果內容是面積，請換算為平方公尺並只保留數值部分，最後用字串儲存。
2. 公告土地現值、當期申報地價、移轉現值或原規定地價，請換算為「每平方公尺」並只保留數值。
3. 如果有全形的數字、符號，請轉為半形。
4. 如果含有中文的次序、分數，請轉為阿拉伯數字及數學式。
5. 如果內容是日期，請轉換為 %Y-%m-%d 的形式，如果僅有年月，請補充日期為 01（請將民國年轉為西元年）。
6. 如果內容是時間，請轉為 unix timestamp 的形式。
7. 若有可能有多筆資料但無資料時，請視為空陣列。
8. 範圍的幾分之幾請轉換為數學表達式，譬如「10000分之215」請表示為「215/10000」。
9. 範圍如果有「全部」的字樣請直接去除，並表示為「1/1」。
10. 地段和小段分開，譬如「北投區立農段二小段 0028-000地號」請分成「立農段」及「二小段」。
11. 行政區如「北投區立農段二小段 0028-000地號」，請只保留「北投區」的部分。
12. 請保留完整建號，如「0028-000」，請記為「0028-000」。
13. 請將全形的阿拉伯數字及符號全轉為半形。"""
