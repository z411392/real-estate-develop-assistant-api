...
class 建物他項權利部:
    ...
    def from_dict(obj: dict) -> '建物他項權利部':
        ...
        _共同擔保土地 = 共同擔保土地.from_dict(obj.get("共同擔保土地"))
        _共同擔保建物 = 共同擔保建物.from_dict(obj.get("共同擔保建物"))
        _其他登記事項 = str(obj.get("其他登記事項"))
        return 建物他項權利部(_登記次序, _權利種類, _收件日期, _字號, _登記日期, _登記原因, _權利人, _統一編號, _住址, _債權額比例, _擔保債權總金額, _擔保債權種類及範圍, _擔保債權確定日期, _償還日期, _利息或利率, _遲延利息或利率, _違約金, _其他擔保範圍約定, _權利標的, _標的登記次序, _設定權利範圍, _證明書字號, _共同擔保土地, _共同擔保建物, _其他登記事項)
...
class 建物所有權部:
    ...
    def from_dict(obj: dict) -> '建物所有權部':
        ...
        _建物他項權利登記次序 = [建物他項權利登記次序 for 建物他項權利登記次序 in obj.get("建物他項權利登記次序")]
        _其他登記事項 = str(obj.get("其他登記事項"))
        return 建物所有權部(_登記次序, _登記日期, _登記原因, _原因發生日期, _所有權人, _統一編號, _住址, _權利範圍, _權狀字號, _建物他項權利登記次序, _其他登記事項)
...
class 建物標示部:
    ...
    def from_dict(obj: dict) -> '建物標示部':
        _登記日期 = str(obj.get("登記日期"))
        _登記原因 = str(obj.get("登記原因"))
        _建物坐落 = 建物坐落.from_dict(obj.get("建物坐落"))
        _建物門牌 = str(obj.get("建物門牌"))
        _主要用途 = str(obj.get("主要用途"))
        _主要建材 = str(obj.get("主要建材"))
        _層數 = str(obj.get("層數"))
        _層次 = [層次.from_dict(單一層次) for 單一層次 in obj.get("層次")]
        _附屬建物 = [附屬建物.from_dict(單一附屬建物) for 單一附屬建物 in obj.get("附屬建物")]
        _建築完成日期 = str(obj.get("建築完成日期"))
        _共有部分 = [共有部分.from_dict(單一共有部分) for 單一共有部分 in obj.get("共有部分")]
        _主建物資料 = [主建物資料.from_dict(單一主建物資料) for 單一主建物資料 in obj.get("主建物資料")]
        _其他登記事項 = str(obj.get("其他登記事項"))
        return 建物標示部(_登記日期, _登記原因, _建物坐落, _建物門牌, _主要用途, _主要建材, _層數, _層次, _附屬建物, _建築完成日期, _共有部分, _主建物資料, _其他登記事項)
...
class BuildingRegistry:
    ...
    def from_dict(obj: dict) -> 'BuildingRegistry':
        ...
        _建物所有權部 = [建物所有權部.from_dict(每ㄧ建物所有權) for 每ㄧ建物所有權 in obj.get("建物所有權部")]
        _建物他項權利部 = [建物他項權利部.from_dict(每ㄧ建物他項權利) for 每ㄧ建物他項權利 in obj.get("建物他項權利部")]
        return BuildingRegistry(_列印時間, _列印公司, _謄本種類碼, _謄本編號, _謄本核發機關, _資料管轄機關, _行政區, _地段, _小段, _建號, _建物標示部, _建物所有權部, _建物他項權利部)
