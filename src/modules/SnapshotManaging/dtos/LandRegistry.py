...
class 土地他項權利部:
    ...
    def from_dict(data: dict):
        ...
        _共同擔保地號 = [
            ...
            for 每一共同擔保地號 in data.get("共同擔保地號")
        ]
        _共同擔保建號 = [
            共同擔保建號.from_dict(每一共同擔保建號)
            for 每一共同擔保建號 in data.get("共同擔保建號")
        ]
...
class 土地所有權部:
    ...
    def from_dict(data: dict):
        ...
        _當期申報地價年月 = str(data.get("當期申報地價年月"))
        _當期申報地價 = int(data.get("當期申報地價"))
        _前次移轉現值或原規定地價年月 = str(data.get("前次移轉現值或原規定地價年月"))
        _前次移轉現值或原規定地價 = int(data.get("前次移轉現值或原規定地價"))
        _歷次取得權利範圍 = str(data.get("歷次取得權利範圍"))
        _相關他項權利登記次序 = [
            相關他項權利登記次序
            for 相關他項權利登記次序 in data.get("相關他項權利登記次序")
        ]
        _其他登記事項 = str(data.get("其他登記事項"))
        return 土地所有權部(
            _登記次序,
...
class 土地標示部:
    ...
    def from_dict(data: dict):
        ...
        _登記原因 = str(data.get("登記原因"))
        _面積 = str(data.get("面積"))
        _使用分區 = str(data.get("使用分區"))
        _使用地類別 = str(data.get("使用地類別"))
        _公告土地現值年月 = str(data.get("公告土地現值年月"))
        _公告土地現值 = int(data.get("公告土地現值"))
        _地上建物建號 = [
            地上建物建號.from_dict(單一地上建物建號)
            for 單一地上建物建號 in data.get("地上建物建號")
        ]
        _其他登記事項 = str(data.get("其他登記事項"))
        return 土地標示部(
            _登記日期,
...
class LandRegistry:
    ...
    def from_dict(data: dict):
        ...
        _地號 = str(data.get("地號"))
        _土地標示部 = [
            土地標示部.from_dict(每ㄧ土地標示) for 每ㄧ土地標示 in data.get("土地標示部")
        ]
        _土地所有權部 = [
            土地所有權部.from_dict(每ㄧ土地所有權)
            for 每ㄧ土地所有權 in data.get("土地所有權部")
        ]
        _土地他項權利部 = [
            土地他項權利部.from_dict(每ㄧ土地他項權利)
            for 每ㄧ土地他項權利 in data.get("土地他項權利部")
        ]
        return LandRegistry(
            _列印時間,
            _列印公司,
